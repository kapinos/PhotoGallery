...
//  PhotoGallery
//
//  Created by Anastasia on 10/13/17.
...
class CustomNavigationAnimationController: NSObject, UIViewControllerAnimatedTransitioning
{
    ...
    public func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        toView?.layer.anchorPoint = CGPoint(direction == 1 ? 0 : 1, 0.5)
        fromView?.layer.anchorPoint = CGPoint(direction == 1 ? 1 : 0, 0.5)
        ...
        UIView.animate(withDuration: transitionDuration(using: transitionContext), animations: {
            ...
            toView?.layer.transform = CATransform3DIdentity
            fromView?.layer.anchorPoint = CGPoint(0.5, 0.5)
            toView?.layer.anchorPoint = CGPoint(0.5, 0.5)
